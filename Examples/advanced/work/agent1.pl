...
eve(init(var_Id)):-print('Agent init->'),print('Id:'),print(var_Id),nl,assert(myid(var_Id)),var_L is(var_Id-1)mod 4,assert(left(var_L)),var_R is(var_Id+1)mod 4,assert(right(var_R)),start(var_Id).
...
eve(election(var_Id,var_SenderId,var_Phase,var_Distance)):-print(election_chiamato),nl,myid(var_MyId),print(ho_ricevuto_election_da_),print(var_Id),nl,(var_Id>var_MyId->var_FDistance is var_Distance/1,(var_FDistance=:=2**var_Phase->print(fdistance_uguale_duephase),nl,sendReply(var_Id,var_SenderId,var_Phase,var_Distance);var_FDistance<2**var_Phase->print(fdistance_minore_duephase),nl,var_NewDist is var_Distance+1,forwardElection(var_Id,var_MyId,var_Phase,var_NewDist);print(post_instance),var_FDistance>2**var_Phase->print(errore_fd));var_Id==var_MyId->announce(var_MyId);var_Id<var_MyId->print(non_inoltro),nl).
...
sendElectionToAgent(var_AgentCode,var_Id,var_Phase,var_NewDist):-var_X=agent,getAgentName(var_X,var_AgentCode,var_Result),myid(var_MyId),a(message(var_Result,send_message(election(var_Id,var_MyId,var_Phase,var_NewDist),var_Me))).

sendReply(var_Id,var_SenderId,var_Phase,var_Distance):-print(send_reeply_from_user_),print(var_SenderId),nl,left(var_LeftAgent),right(var_RightAgent),(var_SenderId==var_LeftAgent->sendReplyToAgent(var_LeftAgent,var_Id,var_Phase,var_Distance);var_SenderId==var_RightAgent->sendReplyToAgent(var_RightAgent,var_Id,var_Phase,var_Distance)).

sendReplyToAgent(var_AgentCode,var_Id,var_Phase,var_Distance):-print(send_reply_to_agent_),print(var_AgentCode),nl,var_X=agent,getAgentName(var_X,var_AgentCode,var_Result),myid(var_MyId),a(message(var_Result,send_message(reply(var_Id,var_MyId,var_Phase,var_Distance),var_Me))).

eve(reply(var_Id,var_SenderId,var_Phase,var_Distance)):-myid(var_MyId),left(var_LeftAgent),right(var_RightAgent),print(ho_ricevuto_reply_da_),print(var_SenderId),nl,(var_Id==var_MyId->(var_LeftAgent==var_SenderId->replyFromLeft(var_Id,var_Phase,var_Distance);var_RightAgent==var_SenderId->replyFromRight(var_Id,var_Phase,var_Distance));forwardReply(var_Id,var_SenderId,var_Phase)).
...
replyFromRight(var_Id,var_Phase,var_Distance):-right(var_RightAgent),print(sto_eseguendo_reply_from_right),print(var_RightAgent),nl,assert(replyRight(var_RightAgent)),print(sono_qui),nl,left(var_LeftAgent),var_X=agent,getAgentName(var_X,var_LeftAgent,var_Result),print(torno_dietro_con_distance_),print(var_Distance),print(e_phase_),print(var_Phase),nl,a(message(var_Result,send_message(election(var_Id,var_Id,var_Phase,var_Distance),var_Me))).

forwardReply(var_Id,var_SenderId,var_Phase):-left(var_LeftAgent),right(var_RightAgent),(var_SenderId==var_LeftAgent->sendReplyToAgent(var_RightAgent,var_Id,var_Phase);var_SenderId==var_RightAgent->sendReplyToAgent(var_LeftAgent,var_Id,var_Phase)).

goNextPhase(var_Phase):-print(sono_go_next_phase),nl,var_NextPhase is var_Phase+1,right(var_RightAgent),myid(var_MyId),var_X=agent,getAgentName(var_X,var_RightAgent,var_Result),print(mando_a_),print(var_Result),nl,myid(var_MioId),print(io_sono_),print(var_MioId),nl,var_X=agent,getAgentName(var_X,var_MioId,var_This),var_P is var_MioId+0,integer(var_P),print(se_stampo_p_is_intero),nl,integer(var_MioId),print(se_stampo_myid_is_intero),nl,a(message(var_Result,send_message(test(var_MioId),var_This))),a(message(var_Result,send_message(election(3,3,var_NextPhase,1),var_This))),print(ho_inviato).
...
call_cfp(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_462113,var_Ontology,_462117),_462107),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_cfp(var_A,var_C,var_Ag,_462151)),a(message(var_Ag,propose(var_A,[_462151],var_AgI))),retractall(ext_agent(var_Ag,_462189,var_Ontology,_462193)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_461987,var_Ontology,_461991),_461981),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,accept_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_462057,var_Ontology,_462061)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_461875,var_Ontology,_461879),_461869),not(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,reject_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_461931,var_Ontology,_461935)).
...
call_cancel(var_A,var_Ag):-if(clause(high_action(var_A,var_Te,var_Ag),_461439),retractall(high_action(var_A,var_Te,var_Ag)),true),if(clause(normal_action(var_A,var_Te,var_Ag),_461473),retractall(normal_action(var_A,var_Te,var_Ag)),true).
...
refused_message(var_To,var_M):-clause(eliminated_message(var_M,var_To,motivation(conditions_not_verified)),_461255).
...
send_jasper_return_message(var_X,var_S,var_T,var_S0):-clause(agent(var_Ag),_461103),a(message(var_S,send_message(sent_rmi(var_X,var_T,var_S0),var_Ag))).

gest_learn(var_H):-clause(past(learn(var_H),var_T,var_U),_461051),learn_if(var_H,var_T,var_U).

evi(gest_learn(var_H)):-retractall(past(learn(var_H),_460927,_460929)),clause(agente(_460949,_460951,_460953,var_S),_460945),name(var_S,var_N),append(var_L,[46,112,108],var_N),name(var_F,var_L),manage_lg(var_H,var_F),a(learned(var_H)).

cllearn:-clause(agente(_460721,_460723,_460725,var_S),_460717),name(var_S,var_N),append(var_L,[46,112,108],var_N),append(var_L,[46,116,120,116],var_To),name(var_FI,var_To),open(var_FI,read,_460821,[]),repeat,read(_460821,var_T),arg(1,var_T,var_H),write(var_H),nl,var_T==end_of_file,!,close(_460821).
