...
eve(election(var_List)):-unwrap(var_List,var_Id,var_SenderId,var_Phase,var_Distance),handleEvent(var_Id,var_SenderId,var_Phase,var_Distance).

unwrap(var_List,var_Id,var_SenderId,var_Phase,var_Distance):-nth0(0,var_List,var_Id),nth0(1,var_List,var_SenderId),nth0(2,var_List,var_Phase),nth0(3,var_List,var_Distance),print(unwrap_id_),print(var_Id),print(sender_id_),print(var_SenderId),print(phase_),print(var_Phase),print(distance_),print(var_Distance),nl,nl.

handleEvent(var_Id,var_SenderId,var_Phase,var_Distance):-print(election_chiamato_from_),print(var_SenderId),nl,myid(var_MyId),(var_Id>var_MyId->var_FDistance is var_Distance/1,(var_FDistance=:=2**var_Phase->print(fdistance_uguale_duephase),nl,sendReply(var_Id,var_SenderId,var_Phase,var_Distance);var_FDistance<2**var_Phase->print(fdistance_minore_duephase),nl,var_NewDist is var_Distance+1,forwardElection(var_Id,var_SenderId,var_Phase,var_NewDist);print(post_instance),var_FDistance>2**var_Phase->print(errore_fd));var_Id==var_MyId->announce(var_MyId);var_Id<var_MyId->print(non_inoltro),nl,nl,sendTryOtherSideToAgent(var_Id,var_SenderId,var_Phase,var_Distance)).

announce(var_Id):-nl,nl,print(end),nl,nl,a(message(agent4,send_message(leader(var_Id),var_Me))),halt(99).

sendTryOtherSideToAgent(var_AgentCode,var_Id,var_Phase,var_Distance):-print(send_try_other_side_to_agent_),print(var_AgentCode),nl,nl,var_X=agent,getAgentName(var_X,var_AgentCode,var_Result),myid(var_MyId),var_L=[var_Id,var_MyId,var_Phase,var_Distance],a(message(var_Result,send_message(otherSide(var_L),var_Me))).

eve(otherSide(var_List)):-unwrap(var_List,var_Id,var_SenderId,var_Phase,var_Distance),myid(var_MyId),left(var_LeftAgent),right(var_RightAgent),print(ho_ricevuto_other_side_da_),print(var_SenderId),nl,(var_Id==var_MyId->startNextSide(var_Id,var_Phase);sendTryOtherSideToAgent(var_Id,var_SenderId,var_Phase,var_Distance)).

startNextSide(var_Id,var_Phase):-left(var_LeftAgent),var_X=agent,getAgentName(var_X,var_LeftAgent,var_Result),var_L=[var_Id,var_Id,var_Phase,1],a(message(var_Result,send_message(election(var_L),var_Me))).

forwardElection(var_Id,var_SenderId,var_Phase,var_NewDist):-left(var_LeftAgent),right(var_RightAgent),(var_SenderId==var_LeftAgent->sendElectionToAgent(var_RightAgent,var_Id,var_Phase,var_NewDist);var_SenderId==var_RightAgent->sendElectionToAgent(var_LeftAgent,var_Id,var_Phase,var_NewDist)).

sendElectionToAgent(var_AgentCode,var_Id,var_Phase,var_NewDist):-var_X=agent,getAgentName(var_X,var_AgentCode,var_Result),myid(var_MyId),print(forward_election_from_),print(var_MyId),print(to),print(var_AgentCode),nl,print(forward_content_is_id_),print(var_Id),print(phase_),print(var_Phase),print(dist),print(var_NewDist),nl,var_L=[var_Id,var_MyId,var_Phase,var_NewDist],a(message(var_Result,send_message(election(var_L),var_Me))).
...
sendReplyToAgent(var_AgentCode,var_Id,var_Phase,var_Distance):-print(send_reply_to_agent_),print(var_AgentCode),nl,nl,var_X=agent,getAgentName(var_X,var_AgentCode,var_Result),myid(var_MyId),var_L=[var_Id,var_MyId,var_Phase,var_Distance],a(message(var_Result,send_message(reply(var_L),var_Me))).

eve(reply(var_List)):-unwrap(var_List,var_Id,var_SenderId,var_Phase,var_Distance),myid(var_MyId),left(var_LeftAgent),right(var_RightAgent),print(ho_ricevuto_reply_da_),print(var_SenderId),nl,(var_Id==var_MyId->(var_LeftAgent==var_SenderId->replyFromLeft(var_Id,var_Phase,var_Distance);var_RightAgent==var_SenderId->replyFromRight(var_Id,var_Phase,var_Distance));forwardReply(var_Id,var_SenderId,var_Phase,var_Distance)).

replyFromLeft(var_Id,var_Phase,var_Distance):-left(var_LeftAgent),right(var_RightAgent),print(vado_nella_next),nl,goNextPhase(var_Phase).

replyFromRight(var_Id,var_Phase,var_Distance):-right(var_RightAgent),print(sto_eseguendo_reply_from_right),print(var_RightAgent),nl,left(var_LeftAgent),var_X=agent,getAgentName(var_X,var_LeftAgent,var_Result),myid(var_MyId),print(torno_dietro_invio_handle_event_from),print(var_MyId),print(to),print(var_Result),print(with_val_id_),print(var_Id),print(distance_),print(var_Distance),print(and_phase_),print(var_Phase),nl,nl,var_L=[var_Id,var_Id,var_Phase,1],a(message(var_Result,send_message(election(var_L),var_Me))).
...
goNextPhase(var_Phase):-print(sono_go_next_phase),nl,var_NextPhase is var_Phase+1,right(var_RightAgent),myid(var_MyId),var_X=agent,getAgentName(var_X,var_RightAgent,var_Result),print(mando_a_),print(var_Result),nl,myid(var_MioId),print(io_sono_),print(var_MioId),nl,var_L=[var_MioId,var_MioId,var_NextPhase,1],a(message(var_Result,send_message(election(var_L),var_Me))).
...
call_cfp(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_499529,var_Ontology,_499533),_499523),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_cfp(var_A,var_C,var_Ag,_499567)),a(message(var_Ag,propose(var_A,[_499567],var_AgI))),retractall(ext_agent(var_Ag,_499605,var_Ontology,_499609)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_499403,var_Ontology,_499407),_499397),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,accept_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_499473,var_Ontology,_499477)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_499291,var_Ontology,_499295),_499285),not(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,reject_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_499347,var_Ontology,_499351)).
...
call_cancel(var_A,var_Ag):-if(clause(high_action(var_A,var_Te,var_Ag),_498855),retractall(high_action(var_A,var_Te,var_Ag)),true),if(clause(normal_action(var_A,var_Te,var_Ag),_498889),retractall(normal_action(var_A,var_Te,var_Ag)),true).
...
refused_message(var_To,var_M):-clause(eliminated_message(var_M,var_To,motivation(conditions_not_verified)),_498671).
...
send_jasper_return_message(var_X,var_S,var_T,var_S0):-clause(agent(var_Ag),_498519),a(message(var_S,send_message(sent_rmi(var_X,var_T,var_S0),var_Ag))).

gest_learn(var_H):-clause(past(learn(var_H),var_T,var_U),_498467),learn_if(var_H,var_T,var_U).

evi(gest_learn(var_H)):-retractall(past(learn(var_H),_498343,_498345)),clause(agente(_498365,_498367,_498369,var_S),_498361),name(var_S,var_N),append(var_L,[46,112,108],var_N),name(var_F,var_L),manage_lg(var_H,var_F),a(learned(var_H)).

cllearn:-clause(agente(_498137,_498139,_498141,var_S),_498133),name(var_S,var_N),append(var_L,[46,112,108],var_N),append(var_L,[46,116,120,116],var_To),name(var_FI,var_To),open(var_FI,read,_498237,[]),repeat,read(_498237,var_T),arg(1,var_T,var_H),write(var_H),nl,var_T==end_of_file,!,close(_498237).
