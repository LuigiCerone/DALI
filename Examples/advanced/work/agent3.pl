...
eve(election(var_List)):-unwrap(var_List,var_Id,var_SenderId,var_Phase,var_Distance),print(election_chiamato_from_),print(var_SenderId),nl,myid(var_MyId),(var_Id>var_MyId->var_FDistance is var_Distance/1,(var_FDistance=:=2**var_Phase->print(fdistance_equals_duephase),nl,sendReply(var_Id,var_SenderId,var_Phase,var_Distance);var_FDistance<2**var_Phase->print(fdistance_less_than_duephase),nl,var_NewDist is var_Distance+1,forwardElection(var_Id,var_SenderId,var_Phase,var_NewDist);var_FDistance>2**var_Phase->print(error_fd));var_Id==var_MyId->announce(var_MyId);var_Id<var_MyId->print(dont_forward),nl,nl).
...
announce(var_Id):-nl,nl,print(end),nl,nl,a(message(agent4,send_message(leader(var_Id),var_Me))).
...
eve(reply(var_List)):-unwrap(var_List,var_Id,var_SenderId,var_Phase,var_Distance),myid(var_MyId),left(var_LeftAgent),right(var_RightAgent),print(received_a_reply_from_),print(var_SenderId),nl,(var_Id==var_MyId->(var_LeftAgent==var_SenderId->replyFromLeft(var_Id,var_Phase,var_Distance);var_RightAgent==var_SenderId->replyFromRight(var_Id,var_Phase,var_Distance));forwardReply(var_Id,var_SenderId,var_Phase,var_Distance)).

replyFromRight(var_Id,var_Phase,var_Distance):-right(var_RightAgent),print(exectuing_reply_from_right),print(var_RightAgent),nl,left(var_LeftAgent),var_X=agent,getAgentName(var_X,var_LeftAgent,var_Result),myid(var_MyId),print(going_back_and_sending_election_from),print(var_MyId),print(to),print(var_Result),print(with_val_id_),print(var_Id),print(distance_),print(var_Distance),print(and_phase_),print(var_Phase),nl,nl,var_L=[var_Id,var_Id,var_Phase,1],a(message(var_Result,send_message(election(var_L),var_Me))).

replyFromLeft(var_Id,var_Phase,var_Distance):-left(var_LeftAgent),right(var_RightAgent),print(going_to_next_fase),nl,goNextPhase(var_Phase).
...
goNextPhase(var_Phase):-var_NextPhase is var_Phase+1,right(var_RightAgent),myid(var_MyId),var_X=agent,getAgentName(var_X,var_RightAgent,var_Result),var_L=[var_MyId,var_MyId,var_NextPhase,1],nl,nl,print(new_phase_is_starting),nl,nl,a(message(var_Result,send_message(election(var_L),var_Me))).
...
call_cfp(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_535951,var_Ontology,_535955),_535945),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_cfp(var_A,var_C,var_Ag,_535989)),a(message(var_Ag,propose(var_A,[_535989],var_AgI))),retractall(ext_agent(var_Ag,_536027,var_Ontology,_536031)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_535825,var_Ontology,_535829),_535819),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,accept_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_535895,var_Ontology,_535899)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_535713,var_Ontology,_535717),_535707),not(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,reject_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_535769,var_Ontology,_535773)).
...
call_cancel(var_A,var_Ag):-if(clause(high_action(var_A,var_Te,var_Ag),_535277),retractall(high_action(var_A,var_Te,var_Ag)),true),if(clause(normal_action(var_A,var_Te,var_Ag),_535311),retractall(normal_action(var_A,var_Te,var_Ag)),true).
...
refused_message(var_To,var_M):-clause(eliminated_message(var_M,var_To,motivation(conditions_not_verified)),_535093).
...
send_jasper_return_message(var_X,var_S,var_T,var_S0):-clause(agent(var_Ag),_534941),a(message(var_S,send_message(sent_rmi(var_X,var_T,var_S0),var_Ag))).

gest_learn(var_H):-clause(past(learn(var_H),var_T,var_U),_534889),learn_if(var_H,var_T,var_U).

evi(gest_learn(var_H)):-retractall(past(learn(var_H),_534765,_534767)),clause(agente(_534787,_534789,_534791,var_S),_534783),name(var_S,var_N),append(var_L,[46,112,108],var_N),name(var_F,var_L),manage_lg(var_H,var_F),a(learned(var_H)).

cllearn:-clause(agente(_534559,_534561,_534563,var_S),_534555),name(var_S,var_N),append(var_L,[46,112,108],var_N),append(var_L,[46,116,120,116],var_To),name(var_FI,var_To),open(var_FI,read,_534659,[]),repeat,read(_534659,var_T),arg(1,var_T,var_H),write(var_H),nl,var_T==end_of_file,!,close(_534659).
